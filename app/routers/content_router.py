...
from ..schemas.onboarding_content_schema import OnboardingContentOut   
router = APIRouter(
    prefix="/content",
    tags=["Content Update"]
)
...
@router.put(
    "/{content_id}",
    response_model=OnboardingContentOut,          
    status_code=status.HTTP_200_OK,
    summary="Update onboarding screen content (Admin only)"
)
async def update_onboarding_content(
    ...
    """
    Update existing onboarding content:
    - Title and subtitle via form data
    - Optional image upload (JPEG/PNG, max 3MB)
    """
    content = db.query(OnboardingContent).filter(OnboardingContent.id == content_id).first()
    if not content:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="Onboarding content not found"
        )
